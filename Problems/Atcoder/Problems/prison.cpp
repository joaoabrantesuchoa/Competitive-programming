...
int main() {
	...
	int qtd = 0;
	int borda_esquerda = INT_MIN;
	int borda_direita = INT_MAX;

	for (int i = 0; i < m; i++) {
		cin >> l >> r;

		borda_esquerda = max(l, borda_esquerda);
		borda_direita = min(r, borda_direita);
	}

	if (borda_direita >= borda_esquerda) {
		cout << abs(borda_direita - borda_esquerda + 1) << nl;
	} else {
		cout << 0 << nl;
	}
